...
It's been busy months and we've shipped a lot of new features and improvements. Let's recap them one by one.

## Polar Integration

Managing tool usage and billing is a thing, and we've got you covered with the Polar. You can add paywalls to your tools and track usage for billing in a few steps.
...
You can return React components for interactive, composable widgets. xmcp renders the component to HTML and generates a widget resource automatically.
...
xmcp is fully integrated with the Apps SDK from OpenAI, so you can easily build and run app in ChatGPT.
...
But not only that, bundle sizes decreased by a third. This enables us to have a better developer experience and more robust tools.


## Connect to external MCP, discover their tools and use them in yours

One of our most recent released features is the ability to connect to external MCP servers, discover their tools and use them. 
...
```
<Callout variant="info" icon="info" title="CLI">
STDIO need to be installed in your environment for it to work
</Callout>


Afterwards, you can generate the clients with the CLI:
```bash
...
- You can configure CORS middleware to allow requests from external domains, headers, methods and credentials.
- Structured Content output is now supported, tools can return structured content alongside UI for better responses.
- Bun support.
