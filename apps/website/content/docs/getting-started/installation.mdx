  ...
  tabs={[
    {
      label: "pnpm",
      value: "pnpm",
      content: "pnpm add xmcp zod@^3.25.76 && pnpm add -D @xmcp-dev/compiler",
    },
    {
      label: "npm",
      value: "npm",
      content: "npm i xmcp zod@^3.25.76 && npm i -D @xmcp-dev/compiler",
    },
    {
      label: "yarn",
      value: "yarn",
      content: "yarn add xmcp zod@^3.25.76 && yarn add -D @xmcp-dev/compiler",
    },
    {
      label: "bun",
      value: "bun",
      content: "bun add xmcp zod@^3.25.76 && bun add -D @xmcp-dev/compiler",
    },
  ]}
  defaultTab="pnpm"
/>

`xmcp` contains the runtime used by your built server. The development-only
`@xmcp-dev/compiler` package provides the `dev`, `build`, and `create` commands;
keep both packages on matching versions. Production installs can omit development
dependencies because xmcp build output is self-contained.
...
If `xmcp dev` or `xmcp build` reports that the compiler is missing, install the
matching compiler version with `npm i -D @xmcp-dev/compiler` (or the equivalent
