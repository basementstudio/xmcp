...
export default async function greet({ name }: InferSchema<typeof schema>) {
  ...
  const result = `Hello, ${name}! Your Clerk user ID is ${session.userId}`;
...
export default async function getOrganization() {
  ...
  if (!session.organizationId) {
    return {
      content: [{
        type: "text",
        text: "User is not in an organization",
      }],
    };
  }
...
After authentication is configured, users will be redirected to Clerk's hosted UI when establishing a connection to your MCP server. The OAuth flow is handled entirely by Clerk, and your server only needs to validate the resulting tokens.
