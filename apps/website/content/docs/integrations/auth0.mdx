...
xmcp automatically infers the tool's scope from its name in the metadata, resulting in `tool:<metadata.name>`. The Auth0 permission should match the tool name.
