...
- **`publicTools`**: (Optional) Array of tool names that don't require `tool:<name>` permission. These tools are accessible to any authenticated user.
...
By default, all tools require a `tool:<tool-name>` permission to access. This ensures fine-grained access control where each tool must be explicitly granted to users.

To use protected tools:
...
Public tools are accessible by any authenticated user without requiring specific `tool:<name>` permissions. To make tools public, add them to the `publicTools` array in your middleware configuration:
...
export default auth0Provider({
  ...
  clientId: process.env.CLIENT_ID!,
  clientSecret: process.env.CLIENT_SECRET!,
  publicTools: ["greet", "ping"],  // These tools are accessible to any authenticated user
});
```
...
<Callout variant="info">
  Public tools still require authentication. Users must have a valid Auth0 token to access them, but they don't need specific `tool:<name>` permissions.
</Callout>
