...
export default requireScopes(async ({ name }: InferSchema<typeof schema>) => {
...
<Callout variant="info">
  Scope inference: the tool scope defaults to `tool:<fileName>` (e.g., `greet.ts` -> `tool:greet`). If you need a different scope name than the file, pass it explicitly: `requireScopes(["tool:custom"], handler)`. The scope you configure in Auth0 must match the tool name you expose.
</Callout>
