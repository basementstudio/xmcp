...
Auth0 supports protecting tools by requiring a permission to access. Follow these steps:

1. In your Auth0 API, created in step 4, add the permission with the following syntext `tool:<tool-name>` (e.g., `tool:whoami`)
2. Assign this permission to roles/users who should have access

Now when users without the permission will see: "You don't have permission to use the 'tool-name' tool."
...
Token Vault allows your MCP tools to access external APIs (Google, GitHub, Slack, etc.) on behalf of authenticated users. Use `getAuth0Client()` to get access tokens for federated connections.
...
<Callout variant="info">
  Token Vault requires the `management` configuration in your middleware. Auth0 handles token refresh automatically—you never need to manage external provider refresh tokens.
</Callout>

### Supported Connections

Token Vault works with social and enterprise connections configured in your Auth0 tenant, including:
