...
- In your **Dashboard** go to **Applications** and create a M2M applicatino
...
The `getManagement()` function provides typed methods for all Auth0 Management operations and is only available when the `management` configuration is provided. See the [official SDK](https://www.npmjs.com/package/auth0) for more details.
