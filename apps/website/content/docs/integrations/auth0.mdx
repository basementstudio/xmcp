...
export default async function myTool() {
  ...
  // Use client for token operations
  return "Done";
}
```
...
- **`verifyAccessToken(options)`** - Verify JWT tokens (used internally)
- **`getAccessTokenForConnection(options)`** - Token Vault - get external provider tokens
- **`getTokenByExchangeProfile(token, options)`** - Custom Token Exchange (RFC 8693)

#### Token Vault (External Provider Tokens)

Use Token Vault to get access tokens for external providers like Google, GitHub, or LinkedIn. This allows your MCP tools to call external APIs on behalf of the authenticated user.

```typescript
import { getAuth0Client, getAuthInfo } from "@xmcp-dev/auth0";

export default async function getGoogleCalendar() {
  const client = getAuth0Client();
  const authInfo = getAuthInfo();

  // Get Google access token from Token Vault
...
<Callout variant="warning">
  Token Vault requires the `management` configuration to be set (clientId and clientSecret). These credentials are used for both the Management API and Token Vault operations.
</Callout>

<Callout variant="info">
  Token Vault also requires additional Auth0 setup. See [Token Vault Setup](#token-vault-setup) below.
</Callout>

#### Custom Token Exchange (RFC 8693)

Exchange tokens for different API audiences while preserving user identity:
...
export default async function callBackendApi() {
  ...
  // Exchange token for a different API audience
  const result = await client.getTokenByExchangeProfile(authInfo.token, {
...
<Callout variant="warning">
  Custom Token Exchange requires the `management` configuration and a Token Exchange Profile configured in Auth0.
</Callout>
...
export default async function myTool() {
  ...
  const { data: user } = await client.users.get(authInfo.extra.sub);
...
Token Vault allows your MCP tools to access external provider APIs (Google, GitHub, etc.) on behalf of authenticated users. This requires additional Auth0 configuration.

### Step 1: Enable Token Vault for a Connection

1. Go to **Auth0 Dashboard** → **Authentication** → **Social**
2. Select the connection (e.g., Google, GitHub, LinkedIn)
3. Go to **Advanced Settings** → **Token Vault**
4. Enable **"Store Identity Provider Access Token"**
5. Save changes

### Step 2: Configure Required Scopes

When users authenticate via the social connection, Auth0 needs to request the appropriate scopes from the provider:

1. In the connection settings, go to **Permissions**
2. Add the scopes your tools need (e.g., `https://www.googleapis.com/auth/calendar.readonly` for Google Calendar)
3. Save changes

### Step 3: Use Token Vault in Tools

```typescript title="src/tools/get_connection_token.ts"
import { z } from "zod";
import type { InferSchema, ToolMetadata } from "xmcp";
import { getAuth0Client, getAuthInfo } from "@xmcp-dev/auth0";

export const schema = {
  connection: z.string().describe("Connection name (e.g., 'google-oauth2')"),
};

export const metadata: ToolMetadata = {
  name: "get_connection_token",
  description: "Get an access token for an external provider",
};
...
export default async function getConnectionToken({
  ...
  try {
    ...
    return error instanceof Error ? error.message : "Failed to get token";
  }
}
```
...
Token Vault works with social connections that support OAuth 2.0:

- **Google** (`google-oauth2`) - Google APIs (Calendar, Drive, Gmail, etc.)
- **GitHub** (`github`) - GitHub API
- **LinkedIn** (`linkedin`) - LinkedIn API
- **Microsoft** (`windowslive`) - Microsoft Graph API
- **Facebook** (`facebook`) - Facebook Graph API

<Callout variant="info">
  Users must authenticate through the social connection at least once before Token Vault can return tokens. If a user signed up with email/password, they won't have social provider tokens available.
</Callout>
