...
3. For testing, use Base Sepolia testnet and get test USDC from a faucet
...
export default paid(async function analyze({ data }) {
  // Tool logic here
...
Override default pricing for specific tools:
...
- **`price`**: Override default price for this tool
...
```typescript
{
  ...
  network: string;         // EIP155 chain ID (e.g., "eip155:8453")
  asset: string;           // USDC token address
...
4. Client retries the request with payment in the `_meta` field
5. Server verifies payment with the facilitator
6. Tool executes and server settles the payment on-chain
7. Response includes transaction hash in `_meta["x402/payment-response"]`

<Callout variant="warning">
  Tools without the `paid()` wrapper remain free. Only wrapped tools require payment.
</Callout>
...
Here's a complete example with free and paid tools:
