...
The audience identifies your resource to Descope's agentic OAuth endpoints. MCP clients use these endpoints for Dynamic Client Registration and token exchange.
...
- **`audience`**: The audience from your MCP Server resource in the Descope Console (required).
...
Use `getManagementClient()` to fetch a token for a [Descope connection](https://docs.descope.com/agentic-identity-hub/core-components/connections). Connections are OAuth connections configured in Descope where Descope manages the client credentials and token lifecycle on your behalf.
...
- Confirm `DESCOPE_AUDIENCE` matches the audience shown in the console
