...
- `session.role` - User's role
- `session.permissions` - Array of user permissions

### User Properties

Use `getUser()` to fetch the full user profile from WorkOS:
...
`getClient()` gives you access to the full [WorkOS Node SDK](https://workos.com/docs/sdks/node), allowing you to leverage all WorkOS features in your MCP tools.
...
2. **WorkOS AuthKit** - The authorization server that handles login
3. **MCP Server** - The resource server that validates tokens

![sequence-diagram](https://j2fbnka41vq9pfap.public.blob.vercel-storage.com/images/diagram-workos.svg)
...
| Scenario | Action |
|----------|--------|
| CIMD-enabled client | Nothing needed |
