...
2. Set-up SSO: save the `WORKOS_API_KEY` and `WORKOS_CLIENT_ID` and add a Redirect URL (for development we will use `http://127.0.0.1:6274/oauth/callback`, which is the default URL for most MCP clients, for production replace it with your deployed URL)
3. Set-up AuthKit: in this step, save the Auth Kit Domain, it looks like this `https://xxx.authkit.app`
