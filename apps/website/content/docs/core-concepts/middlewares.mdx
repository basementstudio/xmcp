...
Use the `xmcp/auth` module to read validated auth information from the current request in tools, prompts, or resources.
...
If you're writing custom Express middleware, attach validated auth to `req.auth` before calling `next()`. xmcp will expose that value through `xmcp/auth` and tool `extra.authInfo`.
