...
Tool handlers can request MCP sampling through `sample(extra, ...)`.
...
`sample()` throws an error instead of silently degrading.
...
For text-only prompts, `messages[].content` also accepts a string shorthand and
xmcp will normalize it into a text block for the MCP request.
