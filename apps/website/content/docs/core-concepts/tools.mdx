...
Declare an `outputSchema` when your tool returns `structuredContent`:
...
If `outputSchema` is declared, `structuredContent` must conform to it. If your handler returns a primitive (`string` or `number`) and `outputSchema` has exactly one field, xmcp auto-injects it into `structuredContent` using that field.
Undeclared keys are rejected when `outputSchema` is present.
