...
normalize both shapes, or `getSampleTextContent(result)` when you only need the
text blocks.

A runnable example lives under `examples/stdio-transport/src/tools/`, including
...
![Basic `extra.sample()` flow captured in MCPJam](/docs/sampling-helper/mcpjam-basic-sampling.png)

![Tool-enabled sampling flow in MCPJam showing the local tool request](/docs/sampling-helper/mcpjam-tool-loop-request.png)

![Tool-enabled sampling flow in MCPJam after xmcp continued with the tool result](/docs/sampling-helper/mcpjam-tool-loop-result.png)
