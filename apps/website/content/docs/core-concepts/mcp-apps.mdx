...
(`useMcpApp()`) that wraps one shared bridge for the iframe lifetime. React
component cleanup unsubscribes only its observer, so Strict Mode remounts do
not dispose the shared host connection.
