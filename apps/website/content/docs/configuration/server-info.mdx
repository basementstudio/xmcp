...
When `homePage` is not provided, xmcp serves a default MCP console at the `/` endpoint of your HTTP server. The console shows server health, server info, MCP capabilities, available tools, resources, resource templates, and prompts. It also lets you test tool calls directly through the configured MCP endpoint.

Customize the HTML page served at `/` by setting `homePage`.
...
The custom page fully replaces the default console.
