...
Not every server needs complex authorization. Sometimes verifying a valid credential is enough — if you have the key, you have access. Other servers need fine-grained control: read versus write permissions, admin versus regular users, or access restricted to specific tools. The right level of authorization depends on your use case.
...
[API Keys](/docs/authentication/api-key) are static credentials shared between client and server. The client includes the key in a header with each request, and the server validates it before processing. API keys work well for server-to-server communication, internal services, or trusted clients where you control both ends. They provide authentication but not user-level authorization since all requests with the same key share the same identity.

[JSON Web Tokens](/docs/authentication/jwt) are self-contained tokens that carry claims about the user, such as their identity, roles, or permissions. The server validates the token's signature without needing to call an external service. JWTs are useful when you manage authentication externally and want to pass verified user information directly to your MCP server.
