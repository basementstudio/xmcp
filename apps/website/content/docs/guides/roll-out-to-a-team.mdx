...
Verify per-user isolation end-to-end using the [sample app](https://github.com/scalekit-developers/xmcp-scalekit-example), a working reference with middleware wiring, a `whoami` tool to observe per-request identity, and `save_note` / `list_my_notes` tools that demonstrate per-user data isolation. When the wizard prompts you to choose a transport, select **http**.
