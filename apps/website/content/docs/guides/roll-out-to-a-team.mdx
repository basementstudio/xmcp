...
Verify per-user isolation end-to-end using the Scalekit template, or run the same checks against your own deployment.

### Start from the Scalekit template

Bootstrap an authenticated MCP server in one command:

```bash
npx create-xmcp-app --example scalekit
```

The [Scalekit Authentication template](/templates/scalekit) includes the middleware wiring, a `whoami` tool to observe per-request identity, and `save_note` / `list_my_notes` tools that demonstrate per-user data isolation. Run the verification steps below against it or your own implementation.
