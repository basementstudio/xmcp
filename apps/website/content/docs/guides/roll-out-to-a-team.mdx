...
Prove the pattern on the team-notes demo, or run the same checks on your own server after you deploy.

### Start from an in-repo example

A minimal authenticated server using the Scalekit plugin is at `examples/scalekit-http`. It includes the middleware wiring and a `whoami` tool so you can observe per-request identity right away.

The `save_note` / `list_my_notes` pattern from the snippets in this guide can be added on top of that example (or any xmcp server) by scoping storage to `getSession().userId`. The verification steps below work for either the included example or your own implementation.

### Verify per-user isolation

Run these checks against the demo or your deployment:
