...
// Search: agent queries the API spec in a sandboxed microVM
const result = await runInSandbox(code, {
  globals: { spec: specJson },
  timeoutMs: 10000,
});

// Execute: agent chains API calls using fetch() inside the microVM
const result = await runInSandbox(code, {
  globals: { baseUrl: "https://api.example.com", apiKey: process.env.API_KEY },
  timeoutMs: 30000,
});
```

Requires a Vercel account. See the [full sandbox documentation](/docs/integrations/sandbox) for the API reference, security model, auth setup, and pricing.
