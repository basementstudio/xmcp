...
That's it. But there's one more step to get the full token savings.

### 3. Mark tools as `internal`

By default, all tools appear in MCP's `tools/list` response — agents see them upfront. To hide tools from `tools/list` while keeping them callable via `execute`, add `internal: true` to their annotations:
