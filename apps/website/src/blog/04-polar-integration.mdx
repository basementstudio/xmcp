...
- If `eventName` is not set, it will not track usage.
...
![Alt text](./04-meter_details.webp)

The ingest events method will be called automatically detecting the tool name and will increment the usage by 1 on each call. The updated metadata you will see in the dashboard will have this structure:
...
If you have already set up the paywall you don't have to change anything. Simply add the `eventName` to the configuration and you're good to go. The usage will be tracked automatically.
