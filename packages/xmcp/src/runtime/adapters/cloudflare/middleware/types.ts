...
/**
 * Config for built-in cloudflareAuthMiddleware helper.
 */
export interface CloudflareAuthConfig {
  /**
   * Custom token validation function.
   * Return AuthInfo if valid, throw or return null if invalid.
   */
  validateToken: (
    token: string,
    env: Env
  ) =>
    | Promise<Omit<AuthInfo, "token"> | null>
    | Omit<AuthInfo, "token">
    | null;
  ...
  /**
   * Whether auth is required. If false, allows unauthenticated requests.
   * @default true
   */
  required?: boolean;
}
